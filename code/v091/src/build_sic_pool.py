...
from sic_stress_bench import (SCENARIOS,DV_DT_SCENARIOS,THERMAL_SCENARIOS,PROTECTION_SCENARIOS,
                              DESAT_BLANK_SCENARIOS,MILLER_SCENARIOS,MILLER_LOW_SCENARIOS,MILLER_HIGH_SCENARIOS)
ASSET=ROOT/'v091/results/sic-asset-01/sic_gate_ctrl_v3';OUT=ROOT/'v091/results/sic-pool-01'
...
def main():
    ...
    N=len(physical);runner=Runner(cfg,OUT,'sic_pool_v3',ASSET/'baseline.vvp',physical,stream=False)
    ...
    for i in range(N):
        rows=[]
        for ev in events:
            r=runner.run(ev['seed'],target=i,cycle=ev['cycle'],domain='sic_pool_v3')
        ...
        obs.append(rows)
        if i%10==9:print('SIC_V3_POOL_PROGRESS',i+1,'/',N,flush=True)
    ...
    print('SIC_V3_POOL_PASS',json.dumps({k:summary[k] for k in ['total_events','fail_events','nonzero_bits','mean_failure_rate','max_failure_rate','elapsed']}),flush=True)
    print('SIC_V3_POOL_CLASSES',json.dumps(by_class,sort_keys=True),flush=True)
    print('SIC_V3_POOL_HAZARDS',json.dumps(hazard_events,sort_keys=True),flush=True)
    for x in summary['top_bits'][:15]:print('SIC_V3_TOP',x,flush=True)
