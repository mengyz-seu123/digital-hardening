import sys,json,time,hashlib,statistics,collections
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]
sys.path[:0]=[str(ROOT/'v095/src'),str(ROOT/'v030/src'),str(ROOT/'src')]
import hardware as h
from fault_runner import Runner
from ext_power_bench import SCENARIOS
ASSET=ROOT/'v095/results/ext-asset-01/ext_motor_axis_ctrl';OUT=ROOT/'v095/results/ext-pool-01'
M=32
...
def main():
    OUT.mkdir(parents=True,exist_ok=False);cfg=json.loads((ASSET/'config.json').read_text());physical=json.loads((ASSET/'baseline_bits.json').read_text());N=len(physical);runner=Runner(cfg,OUT,'ext_power_pool',ASSET/'baseline.vvp',physical,stream=False)
    # M=32 is required by the frozen v040 finite-pool service/certificate. Two
    ...
    for i in range(N):
        ...
        obs.append(arm)
        if i%10==9:print('V095_EXT_POOL_PROGRESS',i+1,'/',N,flush=True)
