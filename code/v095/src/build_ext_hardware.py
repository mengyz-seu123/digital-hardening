...
ASSET=ROOT/'v095/results/ext-asset-01/ext_motor_axis_ctrl';OUT=ROOT/'v095/results/ext-hardware-01';DESIGN='ext_motor_axis_ctrl'
...
def main():
    ...
    catalogue3.PERIODS[DESIGN]=10;catalogue3.ASSETS[DESIGN]=ASSET;cat=catalogue3.generate(DESIGN,ASSET);cat['role']='independent external-spec power-controller transfer case; label-free catalogue before fault labels';h.dump(OUT/'catalog.json',cat)
    h.dump(OUT/'protocol.json',dict(status='frozen',fault_labels_used=False,candidate_generator='v030 catalogue3.generate unchanged',area_budget=.2,normal_screen_scenarios=SCENARIOS,candidate_count=len(cat['plans'])))
    ...
    h.dump(OUT/'summary.json',summary);print('V095_EXT_HW_COMPLETE',json.dumps(summary,sort_keys=True),flush=True)
