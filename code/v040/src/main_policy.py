...
from information_baselines import information_scores
...
def run(method,mask,service,seed,epsilon=.02,M=32,batch=8,quota=1.,max_actions=30000):
    ...
    for step in range(max_actions):
        begin=time.perf_counter();L=f/M;U=(f+M-n)/M;mu=(f+1)/(n+2)
        ...
        if method in ['MC-Entropy','MC-KG']:
            fs,hs=information_scores(method,mask,n,f,status,observations,service,rng)
            fs[n>=M]=-1;hs[status!=-1]=-1;hs[gaps[S]<=epsilon+1e-12]=-1
            bit=int(np.argmax(fs)) if len(available) else None
            if len(candidates):
                js=int(np.argmax(hs))
                if bit is None or hs[js]/cs>=fs[bit]/cf:action='screen'
            if bit is not None and fs[bit]<=1e-12:bit=int(diff[np.argmin(n[diff])])
        elif method=='Optimistic-Upper':
            j=int(live[np.argmax((mask@U)[live])])
            if status[j]==-1:action='screen';js=j
            else:
                S=j;challenger=int(live[np.argmax(gaps[S,live])])
                diff=np.flatnonzero((mask[challenger]!=mask[S])&(n<M))
                if not len(diff):diff=available
        ...
        elif method not in ['Dual','Cover','Cover-LP','MC-Entropy','MC-KG']:
            if directed:
