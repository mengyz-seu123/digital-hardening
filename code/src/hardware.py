...
import collections, copy, hashlib, importlib.util, json, os, re, subprocess, time

PROJECT = Path(__file__).resolve().parents[1]
ROOT = PROJECT.parent
LIB = ROOT/'lib/NangateOpenCellLibrary_typical.lib'
_spec = importlib.util.spec_from_file_location('verified_inventory', PROJECT/'snapshots/netlist.py')
_old = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_old)
inventory = _old.inventory
...
def prepare(cfg, folder):
    ...
    dump(folder/'comb.json',dict(modules={'comb_core':comb},creator='DF-Harden v0.2 FF separation'))
...
def formal(cfg, folder, name, implementation, ports, extra=None):
    ...
    if cfg.get('bench')=='crc':
        from formal_state import prove
        return prove(cfg,folder,name,implementation,ports,extra)
