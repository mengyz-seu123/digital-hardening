...
ASSET=ROOT/'results/primary/assets/primary_controller'
OUT=ROOT/'results/primary/hardware';DESIGN='primary_controller'
...
def normal_accept(cfg,folder,name,exe,physical):
    ...
    for seed in seeds:rows.append(r.run(seed,domain='primary_candidate_screen'))
...
def main():
    ...
    candidates.PERIODS[DESIGN]=10;candidates.ASSETS[DESIGN]=ASSET
    cat=candidates.generate(DESIGN,ASSET)
    cat['role']='Primary protection-race case study; deterministic label-free generator frozen before fault labels'
    cat['asset']=str(ASSET);h.dump(OUT/'catalog.json',cat)
    h.dump(OUT/'protocol.json',dict(status='frozen',fault_labels_used=False,candidate_generator='candidate candidates.generate unchanged',
    ...
    for j,plan in enumerate(cat['plans'][1:],1):
        ...
        print('PRIMARY_HW',j,'/',len(cat['plans'])-1,plan['plan_id'],'k',len(sel),'area',round(area_ratio,4),'screen',screen_pass,'proof',cert.get('status'),flush=True)
    ...
    print('PRIMARY_HW_COMPLETE',json.dumps({k:summary[k] for k in ['candidate_count','feasible','infeasible','all_proofs_passed','elapsed']}),flush=True)
