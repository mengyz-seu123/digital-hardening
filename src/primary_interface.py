...
GUARD_RTL = ROOT/'rtl/primary_guard.v'
HEADROOM_BITS = [2,5,6]
...
def frontend(folder: Path, r_ohm: int, filter_pf: int, slew: float,
    ...
    return c.spice(folder, f'''declared direct common-mode feedback model
Vcc vcc 0 3.3
{cm}
Rpull vcc flt {r_ohm}
...
def detection(folder: Path, current_uA: int, short_ns: float, gate_on_ns: float):
    ...
    rows = c.spice(folder, f'''declared DESAT charge envelope
...
def emit_candidate(out: Path, label: str, selected):
    ...
    h.dump(folder/'selected.json',dict(label=label,selected=selected,
...
def expected_core_ff(rtl: Path):
    p=str(rtl)
    if '/headroom/' in p:
        return 55+2*len(HEADROOM_BITS)
    if '/exact/' in p:
        return 55+2*len(EXACT_BITS)
...
def main():
    ...
    for r,flt in itertools.product(RESISTORS,FILTERS):
        ...
        print('PRIMARY_INTERFACE_INTERFACE',r,flt,json.dumps(rows,sort_keys=True),flush=True)
    ...
    for idx,row in enumerate(feasible_interface):
        for start,rtl in [('headroom',headroom_rtl),('exact',exact_rtl)]:
            ...
            candidates.append(z)
            print('PRIMARY_INTERFACE_HW',start,idx,json.dumps(z,sort_keys=True),flush=True)
    ...
    print('PRIMARY_INTERFACE_COMPLETE',json.dumps(summary,sort_keys=True),flush=True)
