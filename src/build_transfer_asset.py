...
OUT=ROOT/'results/transfer/assets';DESIGN='ext_motor_axis_ctrl';RTL=ROOT/'rtl/transfer_controller.v'
...
def main():
    ...
    cfg=dict(design_id=DESIGN,family_id='external-spec-three-phase-power-controller',bench='transfer_power_safety',sources=[str(RTL)],window=144,cycle_min=8,cycle_max=130,transactions=1,output_gates={},
    ...
    print('TRANSFER_BASE_PASS',len(bits),hw['area'],hw['worst_setup_slack_ns'],flush=True)
    ...
    protected=Runner(cfg,positive,'candidate',cexe,cphys).run(witness['seed'],target=pa['physical_id'],cycle=witness['cycle'],domain='ext_power_tmr_positive')
    ...
    h.dump(folder/'asset_ext_power.json',result);print('TRANSFER_ASSET_COMPLETE',json.dumps({k:result[k] for k in ['logical_bits','normal_scenarios','elapsed']}),flush=True)
