...
import stress as c
import primary_data as sic
import transfer_data as ext
import primary_interface as s
import transfer_interface as p0
from headroom_certificate import full_information_summary

PRIMARY_ASSET = ROOT/'results/primary/assets/primary_controller'
TRANSFER_ASSET = ROOT/'results/transfer/assets/ext_motor_axis_ctrl'
PRIMARY_GUARD = ROOT/'rtl/primary_guard.v'
TRANSFER_GUARD = ROOT/'rtl/transfer_guard.v'
EPSILON = .02
AREA_CAP = .20
...
def primary(out: Path, primary_out: Path, liberty: Path):
    ...
    for si,state in enumerate(states):
        ...
        for ii,iface in enumerate(feasible):
            ...
            mapped.append(row)
            print('COMPLETION_PRIMARY',row['completion_id'],round(row['area_ratio'],6),row['under_20'],flush=True)
    ...
    result['sequential_area']=float(old['area_ratio'])
    result['nonworse_than_sequential']=bool(result['oracle']['area_ratio']<=float(old['area_ratio'])+1e-12)
...
def transfer(out: Path, liberty: Path):
    ...
    for si,state in enumerate(states):
        ...
        for ii,iface in enumerate(feasible):
            ...
            row['under_20']=bool(hw['timing_pass'] and hw['tmr_preserved'] and hw['area_ratio']<=AREA_CAP+1e-12)
            mapped.append(row)
            print('COMPLETION_TRANSFER',row['completion_id'],round(row['area_ratio'],6),row['under_20'],flush=True)
...
def main():
    ...
    protocol=dict(status='predeclared_implementation',epsilon=EPSILON,area_cap=AREA_CAP,
                  ...
                  frozen_inputs='declared physical grids, interface catalogue, deadline, cap and nominal campaigns',
    ...
    summary=dict(status='completed',primary=dict(
                    ...
                    sequential_area=pri['sequential_area'],nonworse_than_sequential=pri['nonworse_than_sequential']),
                 ...
                 h3_primary_nonworse=pri['nonworse_than_sequential'],
    ...
    print('COMPLETION_COMPLETE',json.dumps(summary,sort_keys=True),flush=True)
